...
from struct_lmm.utils.sugar_utils import import_one_pheno_from_csv, make_out_dir
...
def entry_point():
    ...
    # analysis options
    parser.add_option(
        "--unique_variants", action="store_true", dest="unique_variants", default=False
    )
    ...
    assert opt.wfile is not None, "Specify low rank random effect file!"
    ...
    # pheno
    y = import_one_pheno_from_csv(opt.pfile, pheno_id=opt.pheno_id, standardize=True)
